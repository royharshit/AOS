/* Hybrid Loader */
/* Algorithm: Spacial Locality, Loads the seg fault page and subsequent page*/
...
int load_elf(const char *filename, int argc, char *argv[], char *envp[]) {
    ...
    for (int i = 0; i < ehdr->e_phnum; i++) {
        if (phdr[i].p_type == PT_LOAD || phdr[i].p_type == PT_TLS || phdr[i].p_type == PT_PHDR) {
            ...
            size_t segment_size = (phdr[i].p_filesz + (PAGE_SIZE - 1)) & ~(PAGE_SIZE - 1);
    ...
    char *stack_top;
    char *stack = mmap((void *)0x7ffffff00000, STACK_SIZE,
